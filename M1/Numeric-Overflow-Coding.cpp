...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    // needed to determine whether or not when calculating the next result an overflow will occur
    T nextResult = start;
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        nextResult += increment;

        // if the next result is less than the current value of result an overflow occurs
        if (nextResult < result) {
            // throw result for overflow error handling
            throw result;
        }
        // otherwise no overflow occurs and we can calculate the new value of result
        else {
            result += increment;
        }
    }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    // needed to determine whether or not when calculating the next result an underflow will occur
    T nextResult = start;
    ...
    {
        // calculate the next result   
        nextResult -= decrement;

        // if the next result is greater than the current value of result an underflow occurs
        if (nextResult > result) {
            // throw result for underflow error handling
            throw result;
        }
        // otherwise no underflow occurs and we can calculate the new value of result
        else {
            result -= decrement;
        }
...
void test_overflow()
{
    ...
    // use try/catch block to detect if an overflow occurs
    try {
        T result = add_numbers<T>(start, increment, steps);

        // if result is not thrown in the add_numbers function call we output the value assigned to result
        std::cout << +result << std::endl;
    }
    // if result is thrown in the add_numbers function call, display an error message
    catch (...) {
        std::cout << "add_numbers failed due to overflow" << std::endl;
    }
...
void test_underflow()
{
    ...
    // use try/catch block to detect if an underflow occurs
    try {
        auto result = subtract_numbers<T>(start, decrement, steps);

        // if result is not thrown in the subtract_numbers function call we output the value assigned to result
        std::cout << +result << std::endl;
    }
    // if result is thrown in the subtract_numbers function call, display an error message
    catch (...) {
        std::cout << "subtract_numbers failed due to underflow" << std::endl;
    }
